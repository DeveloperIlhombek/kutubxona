...
import { uploadImage } from '@/lib/journal/journal' // Rasm yuklash funksiyasini import qilish
...
export default function JournalCreatePage() {
	...
	const { loading, createJournal } = useJournalStore()
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		// Agar rasm tanlangan bo'lsa, lekin yuklanmagan bo'lsa
		if (imageFile && !imageId) {
			toast.error('Iltimos, rasmni yuklang yoki olib tashlang')
			return
		}

		setUploading(true)
		try {
			// JSON object yaratish
			const journalData = {
				name: formData.name,
				issn: formData.issn,
			...
			// Store orqali yuborish
			const success = await createJournal(journalData)
			if (success) {
			...
			toast.error('Jurnal yaratishda xatolik yuz berdi')
		}
	}
			...
			<div className='w-full h-[25vh] bg-gradient-to-r from-indigo-600 via-purple-600 to-indigo-800 dark:from-indigo-700 dark:via-purple-700 dark:to-indigo-900 flex items-center justify-center relative overflow-hidden'>
				...
				<div className='relative z-10 text-center animate-fade-in-up max-w-4xl mx-auto px-6'>
					...
					<p className='text-white/80 mt-2 text-sm md:text-lg'>
						Jurnal malumotlarini toldiring
					</p>
				</div>
			</div>
			...
			<div className='max-w-7xl mx-auto p-4 md:p-6 -mt-12 relative z-10'>
				...
				<form onSubmit={handleSubmit} className='space-y-1'>
					...
					<div className='mt-6 flex justify-end'>
						<Button
							type='submit'
							disabled={uploading || (imageFile !== null && !imageId)}
							className='bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 px-6 py-3'
						>
							{uploading ? (
								<>
									<div className='w-4 h-4 mr-2 animate-spin rounded-full border-2 border-white/20 border-t-white'></div>
