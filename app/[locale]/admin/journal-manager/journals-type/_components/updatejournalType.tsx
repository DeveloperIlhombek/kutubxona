...
import { getJournalTypeId, updateJournalType } from '@/lib/books/journal-type'
import { Loader2, PencilIcon, Save } from 'lucide-react'
import { useEffect, useState } from 'react'
import { toast } from 'sonner'

export function UpdateJournalDialog({
	journalId,
	open,
	onOpenChange,
	onSuccess,
	...
	useEffect(() => {
		if (open && journalId) {
			const loadCountry = async () => {
				try {
					setLoading(true)
					const response = await getJournalTypeId(journalId)
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			setLoading(true)
			await updateJournalType(journalId, formData.name)
			toast.success('Journal updated successfully')
			onOpenChange(false)
			onSuccess()
		} catch (error) {
			toast.error(`Failed to update Journal ${error}`)
		} finally {
			setLoading(false)
		}
	}
							...
							<div className='space-y-2'>
								...
								<div className='relative group'>
									<input
										...
										placeholder='Journal nomini kiriting...'
										required
									/>
