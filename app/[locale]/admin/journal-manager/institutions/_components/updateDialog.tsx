...
import { getInstitutionId, updateInstitution } from '@/lib/books/institution'
...
export function UpdateInstitutionDialog({
	...
	useEffect(() => {
		if (open && institutionId) {
			const loadCountry = async () => {
				try {
					setLoading(true)
					const response = await getInstitutionId(institutionId)
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			setLoading(true)
			await updateInstitution(institutionId, formData.name)
