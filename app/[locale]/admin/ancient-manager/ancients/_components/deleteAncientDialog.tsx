...
import { deleteCountry } from '@/lib/books/countries'
...
export function DeleteAncientDialog({
	...
	const handleDelete = async () => {
		try {
			setLoading(true)
			await deleteCountry(coutryId)
			toast.success(`${coutryName} muvaffaqiyatli o'chirildi`)
			onOpenChange(false)
			onSuccess()
		} catch (error) {
			toast.error(`Fakultetni o'chirishda xatolik: ${error}`)
		} finally {
			setLoading(false)
		}
	}
