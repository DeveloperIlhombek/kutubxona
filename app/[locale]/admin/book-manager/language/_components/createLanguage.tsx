...
import { createLanguage } from '@/lib/books/language'
...
function CreateLanguageDialog({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (response) {
				toast.success(`${formData.name} tili muvaffaqiyatli qo'shildi`)
				if (onCountryAdded) onCountryAdded()
				setFormData({ name: '', code: '' }) // Reset form
			} else {
				throw new Error("Til qo'shishda xatolik yuz berdi")
			}
		} catch (error) {
			toast.error(`Til qo'shishda xatolik: ${error}`)
		} finally {
			setLoading(false)
		}
	}
			...
			<DialogContent className='max-w-md z-999999 bg-gradient-to-br from-slate-50 to-gray-100 dark:from-slate-800 dark:to-slate-900 border border-slate-200/60 dark:border-slate-700/60 shadow-2xl backdrop-blur-sm'>
				<DialogHeader className='space-y-3 pb-6 border-b border-slate-200/50 dark:border-slate-700/50'>
					...
					<DialogDescription className='text-slate-600 dark:text-slate-400 text-base leading-relaxed'>
						Tizimga yangi til qo&apos;shish uchun quyidagi ma&apos;lumotlarni
						kiriting.
					</DialogDescription>
				</DialogHeader>
					...
					<div className='space-y-4'>
						...
						<div className='space-y-2'>
							...
							<p className='text-xs text-slate-500 dark:text-slate-400'>
								1-2 harfli til kodi (ISO format)
							</p>
						</div>
					</div>
