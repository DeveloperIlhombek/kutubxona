...
import { getLanguageById, updateLanguage } from '@/lib/books/language'
...
export function EditLanguageDialog({
	...
	useEffect(() => {
		if (open && countryId) {
			const loadCountry = async () => {
				try {
					setLoading(true)
					const response = await getLanguageById(countryId)
					...
					toast.error(`Failed to load language data ${error}`)
				} finally {
					setLoading(false)
				}
			}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			setLoading(true)
			await updateLanguage(countryId, formData)
