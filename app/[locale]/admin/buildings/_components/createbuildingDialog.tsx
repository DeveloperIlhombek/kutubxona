...
import { createBuilding } from '@/lib/books/building'
import { Loader2, PlusSquareIcon } from 'lucide-react'
import { useState } from 'react'
import { toast } from 'sonner'

function CreateBiuildingDialog({
	trigger,
	onBuildingAdded,
}: {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (response) {
				toast.success(`${formData.name} binosi muvaffaqiyatli qo'shildi`)
				if (onBuildingAdded) onBuildingAdded()
				setFormData({ name: '', location: '' }) // Reset form
			} else {
				throw new Error("Bino qo'shishda xatolik yuz berdi")
			}
		} catch (error) {
			toast.error(`Bino qo'shishda xatolik: ${error}`)
		} finally {
			setLoading(false)
		}
	}
			...
			<DialogContent className='max-w-md z-999999 bg-gradient-to-br from-slate-50 to-gray-100 dark:from-slate-800 dark:to-slate-900 border border-slate-200/60 dark:border-slate-700/60 shadow-2xl backdrop-blur-sm'>
				<DialogHeader className='space-y-3 pb-6 border-b border-slate-200/50 dark:border-slate-700/50'>
					...
					<DialogDescription className='text-slate-600 dark:text-slate-400 text-base leading-relaxed'>
						Tizimga yangi bino qo&apos;shish uchun quyidagi ma&apos;lumotlarni
						kiriting.
					</DialogDescription>
				</DialogHeader>
...
export default CreateBiuildingDialog
