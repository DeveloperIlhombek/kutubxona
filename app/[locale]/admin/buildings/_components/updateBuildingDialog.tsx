...
import { getbuildingById, updateBuilding } from '@/lib/books/building'
...
export function EditBuildingDialog({
		...
		if (open && buildingId) {
			const loadCountry = async () => {
				try {
					...
					toast.error(`Failed to load building data ${error}`)
				} finally {
					setLoading(false)
				}
			}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			toast.error(`Failed to update building ${error}`)
		} finally {
			setLoading(false)
		}
	}
