...
function Debitorspage() {
	...
	const [facultyResponse, setFacultyResponse] = useState<IDebitorResult>()
	...
	const fetchDebitors = async () => {
		try {
			setLoading(true)
			const response = await getDebitors({ pageNumber, pageSize })
							...
							<Table>
								...
								<TableHeader className='bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950 dark:to-purple-950 border-b border-indigo-200/50 dark:border-indigo-800/50'>
									<TableRow>
										<TableCell
											isHeader
											className='px-2 py-4 font-medium text-gray-800 dark:text-gray-200 text-start text-sm'
										>
											#
										</TableCell>
										<TableCell
											isHeader
											className='px-1 py-4 font-medium text-gray-800 dark:text-gray-200 text-start text-sm'
										>
											Kitob nomi
										</TableCell>
										<TableCell
										...
										<TableCell
											isHeader
											className='px-1 py-1 w-20 text-start font-medium text-gray-800 dark:text-gray-200  text-sm'
										>
											Kitob qaytarilganmi
										</TableCell>
									</TableRow>
								...
								<TableBody className='divide-y divide-slate-200/50 dark:divide-slate-700/50'>
									{faculty?.map((item, index) => (
										...
										>
											...
											<TableCell className='px-2 py-4'>
												<span className='text-sm  text-slate-500 dark:text-slate-300 group-hover:text-indigo-700 dark:group-hover:text-indigo-300'>
													{item.pickupDate.toString().slice(0, 10)}
												</span>
											</TableCell>
											<TableCell className='px-2 py-4 text-end'>
												<span className='text-sm text-slate-700 dark:text-slate-300 group-hover:text-indigo-700 dark:group-hover:text-indigo-300'>
													{item.isReturn ? (
														<Check className='text-green-500 w-20 text-end' />
													) : (
														<X className='text-red-500 w-20 text-end' />
													)}
												</span>
											</TableCell>
										</TableRow>
